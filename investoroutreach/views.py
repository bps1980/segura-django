...
from investoroutreach.utils.email_validation import validate_email_zerobounce
...
def trigger_daily_batch(request):
    call_command('send_daily_batch')
    return HttpResponse("Daily batch triggered.")
...
def upload_excel(request):
    if request.method == "POST" and request.FILES.get('excel_file'):
        ...
        for _, row in df.iterrows():
            ...
            # ✅ Validate email
            validation = validate_email_zerobounce(email)
        ...
        messages.success(request, f"✅ Imported {imported} investors.")
...
def send_emails(request):
    ...
    for _, row in df.iterrows():
        ...
        try:
            investor = Investor.objects.get(email=to_email)
...
def build_email_body(company_name, investor_type, location, investor_id):
    ...
    <body style=\"font-family: Arial, sans-serif; color: #333;\">
        ...
        <p>
            <a href=\"https://app.seguramgmt.com/clicktracker/click/{investor_id}\" style=\"color: #007bff;\">Click here to view our investment deck</a>
        </p>
...
def load_from_disk(request):
    df = pd.read_excel('/mnt/c/Users/segura23/Themeforest/SGMT2/segura-django/Investor_Database.xlsx')
    imported = 0

...
class Command(BaseCommand):
    help = "Send daily email batch to 25000 non-US-based investors"
    ...
    def handle(self, *args, **kwargs):
        ...
        if sent_today >= 25000:
            self.stdout.write("\U0001F4EC Daily limit of 25000 emails already reached.")
            return

        # Get non-US-based investors who haven’t been emailed yet
        intl_batch = list(
            Investor.objects.exclude(country__iexact='United States')
            .exclude(email__in=EmailLog.objects.values_list('recipient_email', flat=True))
        )[:25000 - sent_today]

        if not intl_batch:
            self.stdout.write("\u26A0\uFE0F No eligible international investors found.")
            return

        sg = SendGridAPIClient(SENDGRID_API_KEY)
        count = 0

        for inv in intl_batch:
            print(f"\U0001F449 Candidate: {inv.email} ({inv.investor_type}, {inv.country})")
